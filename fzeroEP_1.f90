...
f=epL1-N+0.5D0+dt*(U1+sqrt(hL/Fr2))*sqrt(epxL**2.0D0+epyL**2.0D0)
fo=epL0-N+0.5D0+dt*(U0+sqrt(hL0/Fr2))*sqrt(epxL0**2.0D0+epyL0**2.0D0)
...
SUBROUTINE fzeroEP_brent(j,borde,Fr2,N,dep,epx,epy,h,u,v,zep,dt,epL,epxL,epyL,uL,vL,hL,zepL)
USE coords
implicit none
real (kind=8) :: x1,x2
...
!real (kind=8),dimension(:),allocatable:: ep,epx,h,u
real (kind=8),dimension(N):: ep,epx,epy,h,u,v,zep
integer:: N, iter, itmax, i, borde,j
