...
SUBROUTINE genabsNxi_1_1(fopt,tipo,MC,Nx,Ny,Fr2,dep,etaR,NR,h0,t,dt,q,zt,ep_x,qA,zA)
USE coords
...
call fzero_B(borde,Fr2,Nx,dep,epx,h,u,zep_x,dt,epL,epxL,uL,hL,zepL)
...
END SUBROUTINE genabsNxi_1_1
!----------------------------------------------------------
SUBROUTINE genabsNxi_2_1(fopt,tipo,MC,Nx,Ny,Fr2,dep,qs,hs,h0,Ns,t,dt,q,zt,ep_x,qA,zA)
USE coords

!datos: qs(t), se asume igual para cada nodo del borde, 
!Aproximacion aguas someras
!etaL=señal
!q=qold sin celdas ficticias
!zt=cotas fondo con celdas ficticias
...
implicit none
real (kind=8):: C,t,dt,Fr2,h0,C0,etai, epA,qL, hL, uL, epL, epxL, RL,dep, zepL, epxA,zepA,tauL,epR,epxR,qR,uR,hR,zepR, tauR,RR, Rmas, Rmenos, zmin, AR, VR, Qt, dQ,hR1
...
END SUBROUTINE genabsNxi_2_1

!----------------------------------------------------------
SUBROUTINE genabsNxi_3_1(fopt,tipo,MC,Nx,Ny,Fr2,dep,us,hs,h0,Ns,t,dt,q,zt,ep_x,qA,zA)
USE coords
...
END SUBROUTINE genabsNxi_3_1

! !----------------------------------------------------------
! 
! SUBROUTINE genabsNxi_4_1(fopt,tipo,C,Nx,Ny,Fr2,dep,qs,h0,Ns,t,dt,q,zt,ep_x,qA,zA)
! USE coords
! !ESTA CONDICION NO SE DEBE USAR, HAY QUE REVISARLA
! !Nivel fijo aguas afuera para flujo unidireccional v=0.0D0
! implicit none
! real (kind=8):: C,t,dt,Fr2,h0,C0,etai, epA, hL, uL, epL, epxL, RL,dep, zepL, epxA,zepA,tauL,Rmas, qb,hA,zmin, uA
...
!     call  tau(tipo,C,Fr2,hL,uL,tauL)
!     !print*, 'zepA=',zepA
!     Rmas=RL-0.5D0*dt/Fr2*(zepL*epxL**2.0D0+zepA*epxA**2.0D0)-tauL/hL*dt*epxL
!     !print*, 'Rmas=',Rmas
!     !pause
!     else
!     tauL=0.0D0
...
! 
! 
! END SUBROUTINE genabsNxi_4_1
! 
! !----------------------------------------------------------
! 
! !----------------------------------------------------------
! 
! SUBROUTINE genabsNxi_5_1(fopt,tipo,C,Nx,Ny,Fr2,dep,qs,h0,Ns,t,dt,q,zt,ep_x,qA,zA)
! USE coords
...
! 
! implicit none
! real (kind=8):: C,t,dt,Fr2,h0,C0,etai, epA, hL, uL, epL, epxL, RL,dep, zepL, epxA,zepA,tauL,Rmas, qb,hA,zmin, uA, hminL, AL, TL, qL
! integer:: Nx,Ny,j,i, Ns, fopt, tipo
! integer,dimension(1)::jmin
! real (kind=8), dimension(3,Nx,Ny)::q
! real (kind=8), dimension(Nx+4,Ny+4)::zt
...
! 
! call Saliente(zmin,jmin(1),hminL,AL,TL,qL)
! 
! uL=qL/AL
! hL=AL/TL
! epxL=coordxi(Nx)
