...
borde=1
zumbral=0.1D0!+103.416330D0 !Cambiar este valor
...
ELSE
    ...
    eta1(j)=0.0D0
    qx(j)=0.1667D0*0.07D0!0.255D0*0.01D0!u*h0
    !qx(j)=0.0D0    
    qy(j)=0.0D0
...
!Borde
zA(j)=zt(3,j+2)
h0=0.07D0 !Profundidad Inicial
!Modificado para el test
!h0=1e-7
...
! print*, 'j=',j, 'borde abierto','z=',zA(j)
!  pause
qA(1,j)=h(1)
qA(2,j)=u(1)
qA(3,j)=v(1)
...
!   call fzeroEP(borde,Fr2,Nx,dep,epx,epy,h,u,v,zep_x,dt,epR,epxR,epyR,uR,vR,hR,zepR)
  ...
  call fzeroEP_brent(j,borde,Fr2,Nx,dep,epx,epy,h,u,v,zep_x,dt,epR,epxR,epyR,uR,vR,hR,zepR)
  zepR=0.0D0
...
!qp=qx(j)
qp=0.07D0*0.1667!0.01D0*0.255D0!
!Modificado para el Test
!qp=0.0D0
...
Rmas=RL-dt/Fr2*0.5D0*(zepL*(epxL**2.0D0+epyL**2.0D0)+zepA*(epxA**2.0D0+epyA**2.0D0))-tauL*(epxL+epyL)*dt
!print*, 'Rmas=', Rmas
...
! a1=-3.0D0*RmenosU
! a2=3.0D0*RmenosU**2.0D0
! a3=-qp/K-RmenosU**3.0D0
! ! if (qp>=0.0D0) then
...
! ! end if
! !a4=K*RmenosU**3.0D0-qp
! call grado3(a1,a2,a3,RL)
! 
! !print*, 'RL=', RL 
! 
! !Verificacion
!  cero=RL**3.0D0+a1*RL**2.0D0+a2*RL+a3
! !  print*,'0=', cero, qp
! 
! !Falta agregar friccion!
! 
! Rmas=RL
...
end if !Termina el if de borde abierto si z es mayor que z umbral


! if (j==5) then
! print*, 'j=',j,'hL=', hL, 'hA=',hA, 'h1=',h(1)!,Rmas,Rmenos, U1u
! ! print*, 'epL=', epL
! ! print*,'0=', cero
! ! pause
! end if
